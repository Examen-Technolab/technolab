...
function DoubleTile(props) {
  return (
    <div className="double-tile">
      ...
      <div className={`double-tile__item double-tile__item_type_about ${props.aboutClass || ''}`}>
        ...
        <ul className="double-tile__links-container">
          {
            props.links.map((item, index) => {
              return (
                <LinkButton onClick={item.onClick} key={"linkBtn" + index} color={item.color} link={item.link} target={item.target || '_self'} title={item.linkTitle || 'Перейти'}>{item.title || 'Перейти'}</LinkButton>
              )
            })
          }
        </ul>
